...
class LoRAMultiheadSelfAttention(nn.Module):
    """
    自注意力LoRA封装：对Q/V投影应用LoRA（batch_first=True）
    """
    ...
    def __init__(self, base_mha: nn.Module, r: int = 8, alpha: int = 16, dropout: float = 0.0, freeze_base: bool = True):
        ...
        if freeze_base:
            k_linear.weight.requires_grad_(False)
            if k_linear.bias is not None:
                k_linear.bias.requires_grad_(False)
    ...
    def forward(self, query, key, value, need_weights=False, attn_mask=None, key_padding_mask=None, average_attn_weights=True, is_causal=False):
        B, S, E = query.shape
